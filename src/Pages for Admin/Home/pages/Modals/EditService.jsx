...
import { updateServiceAPI } from "../../../../Services/allAPIs";
import { updateServiceContextResponse } from "../../../../ContextAPI/ContextShare";
...
function EditService({servdet}) {

  const {updateServiceRes,setUpdateServiceRes} = useContext(updateServiceContextResponse)

    const [centredModal, setCentredModal] = useState(false);

    const [switchState, setSwitchState] = useState(servdet.status === "Enabled");
  ...
  const [serviceDetails, setServiceDetails] = useState({
    id:servdet._id,
    serviceName: servdet.serviceName,
    price: servdet.price,
    status: servdet.status,
    description: servdet.description,
  });

  const updateService = async()=>{
    const { id,serviceName, price,  status, description } = serviceDetails;
    ...
    const reqBody = {
      ...
      const token = sessionStorage.getItem("token")
        ...
        // api call
        const result = await updateServiceAPI(id,reqBody,reqHeader);
        console.log(result.status);
            if(result.status == 200){
            setUpdateServiceRes(result.data)
